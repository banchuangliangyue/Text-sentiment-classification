...
from models.transformers import (WEIGHTS_NAME, CONFIG_NAME, BertConfig,
    ...
    for e in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.do_kd:
                ...
                train_acc = (preds == batch[3]).float().mean().detach().cpu().numpy()
            ...
            # pbar(step, {'loss': loss.item(), 'train_acc':train_acc.item()})
            ...
            if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                ...
                if args.local_rank == -1:  # Only evaluate when single GPU otherwise metrics may not average well
                    _,_,val_loss, val_acc = evaluate(args, model, dev_dataset, tokenizer)

                    print('\n=====>Step:{}/{}/{}, lr:{:.5f}, wd:{:.5f}, loss:{:.4f}, val_loss:{:.4f}, acc:{:.4f}, val_acc:{:.4f}'.format(step,
                        e+1, int(args.num_train_epochs), optimizer.param_groups[0]['lr'], optimizer.param_groups[0]['weight_decay'],
                                tr_loss/global_step, val_loss, tr_acc/global_step, val_acc))

                if val_acc > best_val_acc:
                    best_val_acc = val_acc
                    # Save model checkpoint
                    output_model_file = os.path.join(args.output_dir, 'model_'+str(args.id)+'.bin')
                    output_config_file = os.path.join(args.output_dir, 'config_'+str(args.id)+'.bin')

                    # output_dir = os.path.join(args.output_dir, WEIGHTS_NAME)
                    ...
                    # model_to_save.save_pretrained(output_dir)##pytorch_model.bin
                    # torch.save(args, os.path.join(output_dir, 'training_args.bin'))
                    logger.info("Saving model checkpoint to %s", output_dir)
                    tokenizer.save_vocabulary(vocab_path=output_dir)

                print("========epoch:{}, best_val_acc:{:.4f}========".format(e, best_val_acc))
...
def evaluate(args, model, eval_dataset, tokenizer, prefix=""):
    ...
    return results, preds, eval_loss, eval_acc
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    if os.path.exists(cached_features_file):
        ...
        total_fea = features[index]
        train_fea = total_fea[:int(len(index) * 0.8)]
        dev_fea = total_fea[int(len(index) * 0.8):]
        print('train samples:{},  dev samples:{}'.format(len(train_fea), len(dev_fea)))
    else:
    ...
    train_dataset, dev_dataset = gen_dataset(train_fea), gen_dataset(dev_fea)
    return train_dataset, dev_dataset

def load_and_cache_test_examples(args, task, tokenizer, data_type='test'):
    if args.local_rank not in [-1, 0] and not evaluate:
        torch.distributed.barrier()  # Make sure only the first process in distributed training process the dataset,
                                    # and the others will use the cache
    processor = processors[task]()
    ...
    if os.path.exists(cached_features_file):
        ...
        features = torch.load(cached_features_file)
        features = np.array(features)
        print('\n\n====>test samples:{}'.format(len(features)))
    ...
    test_dataset = gen_dataset(features)
    return test_dataset
...
def main():
    ...
    parser.add_argument('--gpu', dest="GPU", type=int, default=7, help='cuda_visible_devices')
    ...
    config = config_class.from_pretrained('bert-base-uncased' ,num_hidden_layers=6, num_labels=num_labels,
    ...
    #Training
    # if args.do_train:
    #     train_dataset, dev_dataset = load_and_cache_examples(args, args.task_name, tokenizer, data_type='train')
    ...
    # tokenizer = tokenizer_class.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
    tokenizer = tokenizer_class.from_pretrained('bert-base-uncased', do_lower_case=args.do_lower_case)
    test_dataset = load_and_cache_test_examples(args, args.task_name, tokenizer, data_type='test')
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            # config = config_class.from_json_file(output_config_file)
            ...
            model.to(args.device)
            result, preds, _, _ = evaluate(args, model, test_dataset, tokenizer, prefix=prefix)
        ...
        dataframe.to_csv("sampleSubmission_"+str(args.id)+".csv", index=False, sep=',')
