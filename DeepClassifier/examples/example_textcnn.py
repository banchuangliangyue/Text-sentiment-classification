...
parser.add_argument("--wd", dest="weights_decay", type=float, metavar='<float>', default=1e-4)
...
parser.add_argument("--emb", dest="update_emb", type=str2bool,metavar='<bool>', default=False)
...
# 定义模型
my_model = TextCNN(embedding_dim=embedding_matrix.shape[1], dropout_rate=0.5, num_class=5,
                   embedding_matrix=embedding_matrix, requires_grads=args.update_emb)
...
bias_list = (param for name, param in my_model.named_parameters() if name[-4:] == 'bias')
others_list = (param for name, param in my_model.named_parameters() if name[-4:] != 'bias')

parameters = [{'params': others_list},
...
my_trainer = Trainer(model_name="textcnn", model=my_model, train_loader=train_loader, dev_loader=dev_loader,
                     test_loader=dev_loader, optimizer=None, loss_fn=loss_fn, save_path=save_path, epochs=args.EPOCHS,
