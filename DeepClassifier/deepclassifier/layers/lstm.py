'''
source:https://github.com/songyingxin/TextClassification/blob/master/models/LSTM.py
'''
...
class LSTM(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers, bidirectional, dropout, batch_first):
        """
        Args:
            input_size: x 的特征维度
            hidden_size: 隐层的特征维度
...
class GRU(nn.Module):
    ...
    def init_params(self):
        for i in range(self.rnn.num_layers):
            nn.init.orthogonal_(getattr(self.rnn, 'weight_hh_l{i}'))
            nn.init.kaiming_normal_(getattr(self.rnn, 'weight_ih_l{i}'))
            nn.init.constant_(getattr(self.rnn, 'bias_hh_l{i}'), val=0)
            nn.init.constant_(getattr(self.rnn, 'bias_ih_l{i}'), val=0)
            getattr(self.rnn, 'bias_hh_l{i}').chunk(4)[1].fill_(1)
            ...
            if self.rnn.bidirectional:
                nn.init.orthogonal_(
                    ...
                    getattr(self.rnn, 'weight_hh_l{i}_reverse'))
                nn.init.kaiming_normal_(
                    getattr(self.rnn, 'weight_ih_l{i}_reverse'))
                    # getattr(self.rnn, f'weight_ih_l{i}_reverse'))
                nn.init.constant_(
                    getattr(self.rnn, 'bias_hh_l{i}_reverse'), val=0)
                    # getattr(self.rnn, f'bias_hh_l{i}_reverse'), val=0)
                nn.init.constant_(
                    getattr(self.rnn, 'bias_ih_l{i}_reverse'), val=0)
                    # getattr(self.rnn, f'bias_ih_l{i}_reverse'), val=0)
                getattr(self.rnn, 'bias_hh_l{i}_reverse').chunk(4)[1].fill_(1)
